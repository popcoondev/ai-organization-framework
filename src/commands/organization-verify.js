...
export async function organizationVerifyCommand(options) {
  ...
  try {
    ...
    if (!activeReleaseRecord) {
      collector.pass("active release manifest", "no active release manifest present");
    } else {
      collector.pass("active release manifest", path.relative(projectRoot, activeReleaseRecord.manifestPath).replaceAll("\\", "/"));
      if (bootstrap.aof_version === activeReleaseRecord.manifest.release_version) {
        collector.pass("active release bootstrap alignment", bootstrap.aof_version);
      } else {
        collector.fail(
          "active release bootstrap alignment",
          `bootstrap.aof_version=${bootstrap.aof_version} does not match manifest.release_version=${activeReleaseRecord.manifest.release_version}`
        );
      }
      const governanceRelease = (organization.contracts ?? []).find((contract) => contract.contract_id === "contract-governance-to-release");
      if (governanceRelease?.artifact_ref === activeReleaseRecord.manifest.release_definition_ref) {
        collector.pass("active release contract alignment", governanceRelease.artifact_ref);
      } else {
        collector.fail(
          "active release contract alignment",
          `contract-governance-to-release artifact_ref=${governanceRelease?.artifact_ref ?? "missing"} does not match manifest.release_definition_ref=${activeReleaseRecord.manifest.release_definition_ref}`
        );
      }
    }
  } catch (error) {
    collector.fail("active release manifest", error instanceof Error ? error.message : String(error));
  }
