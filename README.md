...
現在の stable release は `v3.3.0` である。  
次の in-progress release は `v3.4.0` であり、焦点は次の状態である。

- self-hosting runtime が active release baseline を truthfully 持てること
- release-state drift を runtime surface で検出できること
- bootstrap / governance release contract を runtime-native path で refresh できること
- operator-facing status surfaces が active release manifest に収束できること
