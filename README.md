...
現在の stable release は `v3.2.0` である。  
次の in-progress release は `v3.3.0` であり、焦点は次の状態まで到達している。

- Discovery evidence を Need Validation の upstream governed contract として扱えること
- discovery judgment と discovery handoff を release-grade artifact として扱えること
- `Discovery -> Need Validation -> planning` の self-hosting chain を committed evidence として保てること
- Node 22 LTS を CI-validated runtime lane として明示し、Node 25 の local instability を support boundary に切り分けること
