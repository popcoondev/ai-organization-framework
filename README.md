...
現在は `v1.8.0` までを release 済みで、repo 自体に AOF を適用する self-hosting cycle を進めている。  
次の formal target は `v1.9` で、主題は self-hosting runtime の project memory を manual follow-up ではなく lifecycle-native に維持することである。
