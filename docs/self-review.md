...
`Capability` と別に `Performance Profile`、`Capacity`、必要なら `Fit` を持てるようにした。  
この判断により、AI worker を human duration や headcount に落とし込まず、review load、reliability、parallelism などで比較できる。

## 運用補強

実運用に向けた補助仕様として、次を追加した。

- Human Actor は外部ではなく正式 actor として参加可能
- `Fast Track` と `Deep Path` の routing
- `Max Retries` と `Escalation Target` による deadlock 回避
- active context / summary / snapshot / archive の分離
- markdown に加え JSON companion を持てる decision log
