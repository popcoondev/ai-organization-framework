...
次のどれかに当てはまる場合、`framing only` より `partial runtime` 以上を標準寄りに扱う。

1. uncertainty が高い
2. exploratory work である
3. live experience が判断中心にある
4. repeated confirmation を数回以上回す想定がある
5. `expectation-mismatch` を state として追う必要がある

特に `Value Alignment Loop` を実質的に運用する案件では、recent confirmation window と mismatch state を持ち回る必要があるため、runtime default を強める。
