...
3. `run -> answer -> council-exec(approval reject) -> escalation-resolve(reopen|approve|stop)` の escalation path
4. `run -> answer -> signal(reopen)` の external-signal reopen path

これにより、CI で planning / proposal / review / approval に加えて、human escalation lifecycle の 3 分岐と external change reopen も拾える。
