...
また、prototype には preflight と planning execution、必要なら proposal/review/approval execution、さらに signal reopen と human escalation reopen からの resume execution までまとめて回す `live-verify` command を持たせる。
`verification-bundle.json` には execution policy と artifact inventory だけでなく、provider response metadata の stage summary をまとめた `provider_observability` も残す。
