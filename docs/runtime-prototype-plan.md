...
`verification-bundle.json` には execution policy と artifact inventory だけでなく、branch ごとの結果要約を持つ `branch_outcomes`、verification 前提を固定する `verification_context`、そして provider response metadata の stage summary をまとめた `provider_observability` も残す。
