...
`live-verify` は optional に `verify-archive` を呼べるようにし、raw evidence を出した直後に project-local verification archive を更新できるようにする。必要なら `archive-max-runs` を渡して retention を同時適用できるようにする。
...
history artifact では bundle-level の `verification_recommendation` も比較対象に含め、recommendation action / urgency の drift を追えるようにする。
加えて `summary.recommendation` を持たせ、first/latest recommendation と latest transition を top-level summary から読めるようにする。
...
さらに `verify-archive` で raw verification run を project-local archive に取り込み、`runs/` 配下の raw evidence と `history` / `log` / `lineage` / `dashboard` 系 artifact を同時更新できるようにする。archive には optional な retention (`max-runs`) も持たせ、古い archived run を prune しながら retained set の derived artifact を維持できるようにする。あわせて `verification-archive-index.json` / `.md` を生成し、archive current-state に対する `health_status` / `threshold_status` / alerts / recommendation まで compact に読めるようにする。さらに `verify-archive-log` で `verification-archive-log.json` / `.md` も生成し、archive-level の health / threshold / recommendation / retention transition を時系列で追えるようにする。さらに `verify-archive-dashboard` で `verification-archive-dashboard.json` / `.md` も生成し、archive current-state と archive trend を 1 つの operator-facing rollup に束ねる。
