...
`verification-bundle.json` には execution policy と artifact inventory だけでなく、branch ごとの結果要約を持つ `branch_outcomes` と、provider response metadata の stage summary をまとめた `provider_observability` も残す。
