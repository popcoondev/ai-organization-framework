...
さらに `verify-archive` で raw verification run を project-local archive に取り込み、`runs/` 配下の raw evidence と `history` / `log` / `lineage` / `dashboard` 系 artifact を同時更新できるようにする。archive には optional な retention (`max-runs`) も持たせ、古い archived run を prune しながら retained set の derived artifact を維持できるようにする。
