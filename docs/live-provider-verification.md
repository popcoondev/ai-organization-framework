...
archive current-state と archive trend を 1 つの operator artifact で見たい場合は、`verify-archive-dashboard` を使う。
...
15. optional な archive retention (`--archive-max-runs` / `--max-runs`) の適用
...
project の中に durable に残したい場合は、`verify-archive` が raw verification run を `.aof/artifacts/verification/runs/` に取り込みつつ、`history`、`log`、`lineage`、`dashboard`、`dashboard-log`、`dashboard-index`、`archive-log`、`archive-dashboard` までまとめて更新する。
...
さらに `verification-archive-dashboard.json` と `verification-archive-dashboard.md` も生成でき、archive current-state と archive trend を 1 つの operator-facing rollup として読める。
