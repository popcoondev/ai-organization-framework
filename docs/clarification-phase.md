...
`v1.6` 以降の operating-model では、`Clarification` は一度の質疑で閉じるより、`Value Alignment Loop` の repeated confirmation と接続して扱う。
...
また、確認は毎回新規質問である必要はない。前回と同じ core question を短く再確認してよい。
...
質問選択ロジック、質問 budget、ask と assume の切り分けは [docs/clarification-question-selection.md](docs/clarification-question-selection.md) を参照する。

## Relationship To Value Alignment Loop

`Clarification` は intake 専用の一回限りフェーズではない。  
次のような場合は、短い repeated confirmation として再入してよい。

1. visible artifact を見て user expectation が更新された
2. scale-up 方向に迷いが出た
3. `alignment pulse` で uncertainty が再浮上した

この場合でも、質問を無限に増やすのではなく、同じ core question を再確認し、ぶれた時だけ掘る。
